...
```bash
# 启动 Ranking / Video / Browse（默认 2 实例 + Nginx LB；会先启动名字服务）
./tools/run_servers.sh

# 仅启动名字服务（前台调试）
./tools/run_registry.sh
...
- 三服务 gRPC + **名字服务**（`registry_main` HTTP，`registry_address` 为唯一写死地址）
- 实例 `bind 127.0.0.1:0` 自动端口，启动注册、关闭注销
- 客户端 / 下游通过 `Resolve(ranking|video|browse)` 连 LB 入口（或单实例直连）
- Nginx upstream 由 `registry_ctl setup-lb` 分配端口、注册 LB 入口并生成配置
- `browse_load_test` 多进程压测

## 下一步

快速开始见上文；名字服务已实现，下文为设计备忘。
